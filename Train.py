...
class TrainManager:
    ...
    def train_MLP_one_epoch(self, dataloader, encoder, predictor, optimizer, criterion):
        ...
        for idx in tqdm(range(len(dataloader))):
            ...
            # print(f"Multimodal embeddings dimensions : {encoder_output.shape}")
            # encoder_output = encoder_output.reshape(-1,32,32,24)
            # print(f"Multimodal embeddings new dimensions : {encoder_output.shape}")
    ...
    def train_MLP_image_one_epoch(self, dataloader, encoder, predictor, optimizer, criterion):
        ...
        for img, sen, mri in tqdm(dataloader):
            count += 1
            optimizer.zero_grad()
            img = img.reshape(-1, img.shape[0], img.shape[1], img.shape[2])
            sample = {"image": img, "text_input": sen}
    ...
    def train(self, epoch, brain_type, lr, subj, save_path, train_type='image', model_type='MLP'):
        ...
        optimizer = optim.Adam(predictor.parameters(), lr, weight_decay=1e-1,)
        ...
        if model_type == 'MLP' and train_type == 'text':
            ...
            for index in range(epoch):
                ...
                torch.save(predictor.state_dict(), os.path.join(save_path, f"model_epochN{epoch}_corr{mean_correlation}.pt"))
