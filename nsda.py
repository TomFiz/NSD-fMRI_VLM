...
class NSDAccess(object):
    ...
    def read_betas(self, subject, session_index, trial_index=[], data_type='betas_fithrf_GLMdenoise_RR', data_format='fsaverage', mask=None, read_mean=False):
        ...
        read_mean : bool, optional. If True, read average beta values, by default False.
        ...
        if data_format == 'fsaverage':
            ...
            out_data = nb.load(
                op.join(data_folder, f'meanbeta_session{si_str}.nii.gz')).get_fdata()
    ...
    def read_images(self, image_index, show=False):
        ...
        if not hasattr(self, 'stim_descriptions'):
            self.stim_descriptions = pd.read_csv(
                self.stimuli_description_file, sep=',')
    ...
    def get_subject_trials(self, subject):
        """get_subject_trials
        ...
        -------
        list
            list of trials for this subject
        """
        if not hasattr(self, 'stim_descriptions'):
            self.stim_descriptions = pd.read_csv(
                self.stimuli_description_file, sep=',')
        ...
        trials = []
        subject_name = 'subject' + subject[-1:]
        for i in range(len(self.stim_descriptions)):
            row = self.stim_descriptions.iloc[i]
            if row[subject_name] == 1:
                for column in self.stim_descriptions.columns:
